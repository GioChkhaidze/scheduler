...
std::optional<double> predictedHotDecodeReadyTime(
  ...
  return predictedHotDecodeReadyTime(world, remote, {}, config);
}
...
std::optional<double> predictedHotDecodeReadyTime(
  ...
  const auto contains_hot = [&](const LinkTransferSpec& transfer, RequestState expected_state) {
    if (transfer.stage != TransferStage::Decode || transfer.remote != remote) {
      return false;
    }
    return std::any_of(transfer.rids.begin(), transfer.rids.end(), [&](int rid) {
      const Request& request = getRequest(world, rid);
      return request.state == expected_state && classifyRequest(request) == RequestClass::Hot;
    });
  };

  std::optional<double> result;
  for (const Request& request : world.requests) {
    if (request.remote == remote && request.state == RequestState::ReadyDecodeProc
        && classifyRequest(request) == RequestClass::Hot) {
      result = world.current_time;
      break;
    }
  }
  for (const CommittedLinkTransfer& transfer : world.links.up.committed) {
    if (contains_hot(transfer.transfer, RequestState::WaitingDecodeUpload)) {
      result = std::min(result.value_or(transfer.completes_at), transfer.completes_at);
    }
  }

  struct ProjectedHotUpload {
    FutureTransfer future;
    RequestState expected_state;
  };
  std::vector<ProjectedHotUpload> projected;
  for (const FutureTransfer& future : futureTransfers(world.links, TransferDirection::Up)) {
    projected.push_back({future, RequestState::WaitingDecodePreDone});
  }

  std::uint64_t trigger_order = world.links.next_trigger_order;
  for (const Assignment& assignment : guaranteed_assignments) {
    const std::vector<LinkTransferSpec> transfers =
      triggeredTransfers(world, assignment.task, config.system.num_layers);
    if (transfers.empty()) {
      continue;
    ...
    for (std::size_t index = 0; index < transfers.size(); ++index) {
      if (transfers[index].direction == TransferDirection::Up) {
        projected.push_back({
          {transfers[index], enqueue_at, trigger_order, index}, RequestState::ReadyDecodePre,
        });
      }
    }
    ++trigger_order;
  }
  std::sort(projected.begin(), projected.end(), [](const ProjectedHotUpload& left, const ProjectedHotUpload& right) {
    return std::tie(left.future.enqueue_at, left.future.trigger_order, left.future.within_trigger_order)
      < std::tie(right.future.enqueue_at, right.future.trigger_order, right.future.within_trigger_order);
  });

  double tail = world.links.up.committed_tail;
  for (const ProjectedHotUpload& upload : projected) {
    const FutureTransfer& future = upload.future;
    tail = std::max(tail, future.enqueue_at) + transferTime(config.system, future.transfer.length);
    if (contains_hot(future.transfer, upload.expected_state)) {
      result = std::min(result.value_or(tail), tail);
    }
  }
  return result;
}
...
FinalPrefillDownImpact estimateFinalPrefillDownImpact(
  ...
  std::vector<ProjectedTransfer> projection;
  std::size_t target_count = 0;
  for (const FutureTransfer& future : futureTransfers(world.links, TransferDirection::Down)) {
    std::optional<std::size_t> target;
    if (future.transfer.stage == TransferStage::Decode && hotDeadline(world, future.transfer, config).has_value()) {
      target = target_count++;
    }
    projection.push_back({
      future.transfer, future.enqueue_at, future.trigger_order, future.within_trigger_order, target, false,
    });
  }
  ...
  for (std::size_t assignment_index = 0; assignment_index < candidates.size(); ++assignment_index) {
    ...
    for (std::size_t within = 0; within < transfers.size(); ++within) {
      ...
      const bool candidate = assignment_index == candidate_index;
      std::optional<std::size_t> target;
      if (!candidate && transfer.stage == TransferStage::Decode && hotDeadline(world, transfer, config).has_value()) {
        target = target_count++;
      }
      projection.push_back({
        transfer,
  ...
  const std::vector<double> without_prefill = simulateProjection(
    world, TransferDirection::Down, projection, target_count, false, config.system);
  const std::vector<double> with_prefill = simulateProjection(
    world, TransferDirection::Down, std::move(projection), target_count, true, config.system);
  FinalPrefillDownImpact result;
  for (std::size_t index = 0; index < target_count; ++index) {
    const double saved = with_prefill[index] - without_prefill[index];
    if (saved > result.blocking_saved + 1e-9) {
      result.blocking_saved = saved;
      result.hot_completion_without_prefill = without_prefill[index];
    }
  }
  return result;
}
...
bool hasGuaranteedSchedulerEvent(const WorldState& world) {
  return hasGuaranteedFutureEvent(world);
}
...
std::vector<Assignment> chooseSharedLinkAssignments(
  ...
  return applySharedLinkPolicy(world, num_layers, baseline, config);
}
